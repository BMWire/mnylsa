...
if (empty($_POST['street'])) {
    die('Street name is required');
}
if (empty($_POST['town'])) {
    die('Town is required');
}
if (empty($_POST['county'])) {
    die('County is required');
}
...
if ($_FILES['img']['error']) {
    ...
    move_uploaded_file($_FILES['img']['tmp_name'], 'uploads/artists/' . $_FILES['img']['name']);
    echo "File uploaded successfully";
}

$img_dir = 'uploads/artists/' . $_FILES['img']['name'];
...
$sql = "INSERT INTO artist_details (artist_id, artist_name, profile_image, building, street, town, county, story)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
